...
async def run(picoUnicorn, graphics):
    ...
    num_stars = 50
    stars = [{'x': random.uniform(-width, width), 'y': random.uniform(-height, height), 'speed': random.uniform(0.01, 0.1)} for _ in range(num_stars)]
    ...
    while True:
        ...
        for star in stars:
            ...
            if star['x'] > width // 2 or star['x'] < -width // 2 or star['y'] > height // 2 or star['y'] < -height // 2:
